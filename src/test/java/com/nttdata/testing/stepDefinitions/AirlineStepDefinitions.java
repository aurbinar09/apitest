...
import com.nttdata.testing.tasks.DeletePasenger;
import com.nttdata.testing.tasks.PatchPasenger;
...
public class AirlineStepDefinitions {
    ...
    @Given("el {actor} establece el endpount DELETE para eliminar un pasajero")
    public void elActorEstableceElEndpountDELETEParaEliminarUnPasajero(Actor actor) {
        actor.whoCan(CallAnApi.at("https://api.instantwebtools.net/v1"));
    }


    @When("el envia una solicitud HTTP DELETE con el ID {string}")
    public void elEnviaUnaSolicitudHTTPDELETEConElID(String _id) {
        theActorInTheSpotlight().attemptsTo(DeletePasenger.fromPage(_id));

    }


    @Given("el {actor} establece el endpoint PATCH para actualizar un campo de un pasajero")
    public void elActorEstableceElEndpointPATCHParaActualizarUnCampoDeUnPasajero(Actor actor) {
        actor.whoCan(CallAnApi.at("https://api.instantwebtools.net/v1"));
    }

    @When("el envia una solicitud HTTP PATCH con el ID {string} para cambiar el nombre a{string}")
    public void elEnviaUnaSolicitudHTTPPATCHConElIDParaCambiarElNombreA(String _id, String name) {
        theActorInTheSpotlight().attemptsTo(
                PatchPasenger.withName(_id, name)
        );
    }
}
